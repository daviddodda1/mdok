...
func (d *DockerClient) getContainerIPs(ctx context.Context, targetContainerID string) (map[string]bool, error) {
...
func (d *DockerClient) classifyConnections(ctx context.Context, containerID string, containerIPs map[string]bool) (interContainer, internal, internet int, err error) {
	// Read both IPv4 and IPv6 connection tables
	for _, file := range []string{"/proc/net/tcp", "/proc/net/tcp6"} {
		counts, err := d.readProcNetFile(ctx, containerID, file, containerIPs)
		if err != nil {
...
func (d *DockerClient) readProcNetFile(ctx context.Context, containerID string, procFile string, containerIPs map[string]bool) ([3]int, error) {
...
func (d *DockerClient) getNetworkBreakdown(ctx context.Context, containerID string) (interContainer, internal, internet int) {
	...
	containerIPs, err := d.getContainerIPs(ctx, containerID)
	if err != nil {
		return 0, 0, 0
	}

	// Classify active connections
	interContainer, internal, internet, err = d.classifyConnections(ctx, containerID, containerIPs)
	if err != nil {
		return 0, 0, 0
	}
